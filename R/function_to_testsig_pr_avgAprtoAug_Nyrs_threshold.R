...
testsig_pr_avgAprtoAug_Nyrs_threshold<-function(numsurr=1000, nbin=4, chosen_rad=c(0,250), yr_threshold=40){
  
  if(yr_threshold==40){
    df_pr5<-read.csv(here(paste("RESULTS/summary_spat_syn_for_pr_avgAprtoAug_",chosen_rad[1],
  ...
  for(spi in 1:nrow(df)){
    ...
    nps<-readRDS(here(paste(inputdistloc,"/pr_avgAprtoAug_spatsyn_nbin_",nbin,
    ...
    dat_detrend<-readRDS(here(paste(inputdistloc,"/year_by_site_pr_avgAprtoAug_detrended.RDS",sep="")))
    ...
    for(i in 1:nrow(dfid)){
      ...
      rawdat<-na.omit(rawdat)# if NA for 2019
    ...
    saveRDS(dfid,here(paste(inputdistloc,"/pr_avgAprtoAug_spatsyn_nbin_",nbin,"/corlmcoru_sigres.RDS",sep="")))
...
testsig_pr_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 32)
testsig_pr_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 36)
testsig_pr_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 40)
