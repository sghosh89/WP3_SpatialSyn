...
if(!dir.exists(here("RESULTS/model_phylopath_sig75_0-250km"))){
  dir.create(here("RESULTS/model_phylopath_sig75_0-250km"))
}
...
saveRDS(ct3,here("RESULTS/model_phylopath_sig75_0-250km/consensus_tree_with_edgelength.RDS"))
...
#============== model ===========
call_phylopath_sig75_0_250km<-function(model="tasmax", df){
  ...
  if(model=="tasmax"){
    ...
    myresloc<-here("RESULTS/model_phylopath_sig75_0-250km/model_tasmax")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas"){
    modelsHWI_Tonly<-model_tas
    myresloc<-here("RESULTS/model_phylopath_sig75_0-250km/model_tas")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5"){
    modelsHWI_Tonly<-model_tas5
    myresloc<-here("RESULTS/model_phylopath_sig75_0-250km/model_tas5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas4"){
    modelsHWI_Tonly<-model_tas4
    myresloc<-here("RESULTS/model_phylopath_sig75_0-250km/model_tas4")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas3"){
    modelsHWI_Tonly<-model_tas3
    myresloc<-here("RESULTS/model_phylopath_sig75_0-250km/model_tas3")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else{
    print("define model")
  }
...
#=========================
call_phylopath_sig75_0_250km(model="tasmax", df=df)

call_phylopath_sig75_0_250km(model="tas", df=df)
call_phylopath_sig75_0_250km(model="tas5", df=df)
call_phylopath_sig75_0_250km(model="tas3", df=df)
call_phylopath_sig75_0_250km(model="tas4", df=df)
