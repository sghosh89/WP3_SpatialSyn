...
for(spi in 1:nrow(df)){
  ...
  nps<-readRDS(here(paste("RESULTS/AOU_",givenAOU,"/tas_avgAprtoAug_spatsyn_nbin_",nbin,
  ...
  dat_detrend<-readRDS(here(paste("RESULTS/AOU_", givenAOU,"/year_by_site_tas_avgAprtoAug_detrended.RDS",sep="")))
  ...
  saveRDS(dfid,here(paste("RESULTS/AOU_", givenAOU,"/tas_avgAprtoAug_spatsyn_nbin_",nbin,"/corlmcoru_sigres.RDS",sep="")))
