#=========================================================================================
# We are testing the significance of corlmcoru for tas copula against normal copula
#==========================================================================================
rm(list=ls())
...
testsig_tas_avgAprtoAug_Nyrs_threshold<-function(numsurr=1000, nbin=4, chosen_rad=c(0,250), yr_threshold=40){
  
  if(yr_threshold==40){
    df_tas5<-read.csv(here(paste("RESULTS/summary_spat_syn_for_tas_avgAprtoAug_",chosen_rad[1],
  ...
  df_tas5<-df_tas5%>%dplyr::select(AOU,nL,nU,L,U)
  
  df<-df%>%dplyr::select(AOU,fLU_tas_avgAprtoAug)
  df<-left_join(df,df_tas5,by="AOU") # this is summarized value within 0-250Km distance
  ...
  for(spi in 1:nrow(df)){
    ...
    nps<-readRDS(here(paste(inputdistloc,"/tas_avgAprtoAug_spatsyn_nbin_",nbin,
    ...
    dat_detrend<-readRDS(here(paste(inputdistloc,"/year_by_site_tas_avgAprtoAug_detrended.RDS",sep="")))
    ...
    saveRDS(dfid,here(paste(inputdistloc,"/tas_avgAprtoAug_spatsyn_nbin_",nbin,"/corlmcoru_sigres.RDS",sep="")))
...
testsig_tas_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 32)
testsig_tas_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 36)
testsig_tas_avgAprtoAug_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 40)
