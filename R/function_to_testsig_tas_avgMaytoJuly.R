...
df_tas3<-read.csv(here(paste("RESULTS/summary_spat_syn_for_tas_avgMaytoJuly_",chosen_rad[1],
                             "_",chosen_rad[2],"km_nbin_",nbin,".csv",sep="")))
df_tas3<-df_tas3%>%dplyr::select(AOU,nL,nU,L,U)

df<-read.csv(here("RESULTS/df_abund_climate_spatsyn_0_250km_nbin_4.csv"))
df<-df%>%dplyr::select(AOU,fLU_tas_avgMaytoJuly)
df<-left_join(df,df_tas3,by="AOU") # this is summarized value within 0-250Km distance
...
for(spi in 1:nrow(df)){
  ...
  nps<-readRDS(here(paste("RESULTS/AOU_",givenAOU,"/tas_avgMaytoJuly_spatsyn_nbin_",nbin,
  ...
  dat_detrend<-readRDS(here(paste("RESULTS/AOU_", givenAOU,"/year_by_site_tas_avgMaytoJuly_detrended.RDS",sep="")))
  ...
  saveRDS(dfid,here(paste("RESULTS/AOU_", givenAOU,"/tas_avgMaytoJuly_spatsyn_nbin_",nbin,"/corlmcoru_sigres.RDS",sep="")))
