...
if(!dir.exists(here("RESULTS/model_phylopath_sig95_100-250km"))){
  dir.create(here("RESULTS/model_phylopath_sig95_100-250km"))
}
...
write.table(nm,here("DATA/BirdTree/unique_speciesnameBirdTree_100_250km_nbin4_tailsig95.txt"),quote=F,col.names =F,row.names=F)
# Now we will download 1000 trees with the above 29 species names
...
sigT<-read.nexus(here("DATA/BirdTree/sig95_100_250km_tree-pruner-e6cc469e-a006-409c-a34d-35b468fa4ccd/output.nex"))
...
saveRDS(ct3,here("RESULTS/model_phylopath_sig95_100-250km/consensus_tree_with_edgelength.RDS"))
...
pdf(here("RESULTS/model_phylopath_sig95_100-250km/species_phylogeny_100_250km_absolute_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g1 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_100-250km/species_phylogeny_100_250km_fLU_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g2 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_100-250km/species_phylogeny_100_250km_HWI.pdf"), width = 9, height = 9) # Open a new pdf file
g3 # Write the grid.arrange in the file
dev.off()

#============== model ===========
call_phylopath_sig95_100_250km<-function(model, df){
  ...
  if(model=="tas"){
    ...
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_tas")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5"){
    modelsHWI_Tonly<-model_tas5
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_tas5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr"){
    modelsHWI_Tonly<-model_pr
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_pr")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5"){
    modelsHWI_Tonly<-model_pr5
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_pr5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5_abs_td"){
    modelsHWI_Tonly<-model_tas5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_tas5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5_abs_td"){
    modelsHWI_Tonly<-model_pr5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_100-250km/model_pr5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else{
    print("define model")
  }
