...
if(!dir.exists(here("RESULTS/model_phylopath_sig95_0-250km"))){
  dir.create(here("RESULTS/model_phylopath_sig95_0-250km"))
}
...
saveRDS(ct3,here("RESULTS/model_phylopath_sig95_0-250km/consensus_tree_with_edgelength.RDS"))
...
pdf(here("RESULTS/model_phylopath_sig95_0-250km/species_phylogeny_0_250km_absolute_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g1 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_0-250km/species_phylogeny_0_250km_fLU_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g2 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_0-250km/species_phylogeny_0_250km_HWI.pdf"), width = 9, height = 9) # Open a new pdf file
...
call_phylopath_sig95_0_250km<-function(model, df, ct3){
  ...
  if(model=="tas"){
    ...
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_tas")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5"){
    modelsHWI_Tonly<-model_tas5
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_tas5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr"){
    modelsHWI_Tonly<-model_pr
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_pr")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5"){
    modelsHWI_Tonly<-model_pr5
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_pr5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5_abs_td"){
    modelsHWI_Tonly<-model_tas5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_tas5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5_abs_td"){
    modelsHWI_Tonly<-model_pr5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_0-250km/model_pr5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else{
    print("define model")
  }
