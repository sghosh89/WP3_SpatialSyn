...
dfUT<-df%>%filter(tail=="UT")%>% filter(abs(fLU_ab)>0.9)

gp_list_LT <- vector(mode='list', length=nrow(dfLT))
gp_list_UT <- vector(mode='list', length=nrow(dfUT))
...
for(i in 1:nrow(dfUT)){
  ...
  g1<-g1+theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
               ...
               axis.text.x=element_blank(), 
               axis.text.y=element_blank())
  g1<-g1+geom_point(data=mylonlat,aes(y=Latitude,x=Longitude),col="steelblue3",alpha=0.3,cex=1)+
    #ggtitle(paste("#sites= ",nrow(mylonlat),", species' AOU= ",aou,sep=""))+ 
    ggtitle(paste("AOU= ",aou,sep=""))+
    theme(plot.title = element_text(size = 12, hjust=0.5, vjust=0))
  gp_list_UT[[i]]<-g1
...
pdf(here("RESULTS/allmapsUT.pdf"),width=10,height=7)
grid.arrange(gp_list_UT[[1]], gp_list_UT[[2]], gp_list_UT[[3]],
             gp_list_UT[[4]], gp_list_UT[[5]], gp_list_UT[[6]],
             gp_list_UT[[7]], gp_list_UT[[8]], gp_list_UT[[9]],
             gp_list_UT[[10]], gp_list_UT[[11]], gp_list_UT[[12]],
             gp_list_UT[[13]], gp_list_UT[[14]], gp_list_UT[[15]],
             gp_list_UT[[16]], gp_list_UT[[17]], gp_list_UT[[18]],
             gp_list_UT[[19]], gp_list_UT[[20]], gp_list_UT[[21]],
             gp_list_UT[[22]], gp_list_UT[[23]], gp_list_UT[[24]],
             gp_list_UT[[25]], gp_list_UT[[26]], gp_list_UT[[27]],
             ncol=6)
dev.off()
