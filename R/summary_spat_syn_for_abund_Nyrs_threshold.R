...
call_summary_spat_syn_for_abund_Nyrs_threshold<-function(df, chosen_rad, nbin, yr_threshold=40){
  ...
  for(i in 1:nrow(df)){
    ...
    if(yr_threshold==40){
      ...
      resloc<-here(paste("RESULTS/yr_threshold_",yr_threshold,"/AOU_", givenAOU,"/abundance_spatsyn_nbin_",nbin,sep=""))
...
call_summary_spat_syn_for_abund_Nyrs_threshold(df=df, chosen_rad=chosen_rad, nbin=2, yr_threshold=yr_threshold)
call_summary_spat_syn_for_abund_Nyrs_threshold(df=df, chosen_rad=chosen_rad, nbin=4, yr_threshold=yr_threshold)
#173 sp.
...
#                                                 chosen_rad[1],"_",chosen_rad[2],"km_nbin_",nbin,"_min",yr_threshold,"yr.csv",sep="")))
#df1 has 276 sp. for min 36 yr analysis
#id<-which(df1$npos!=0) # 170 sp. were synchronous
#-------------
yr_threshold<-32
if(yr_threshold==40){
  df<-read.csv(here("DATA/for_BBS/wrangled_data/data1979to2019_abundance_species_w_minimum2sites.csv"))
...
#df1<-read.csv(here(paste("RESULTS/summary_spat_syn_for_abund_",
#                                                 chosen_rad[1],"_",chosen_rad[2],"km_nbin_",nbin,"_min",yr_threshold,"yr.csv",sep="")))
#df1 has 328 sp. for min 32 yr analysis
#
#id<-sum(df1$npos!=0 & df1$nneg==0) # 240 sp. were synchronous
#id<-sum(df1$nneg!=0) # 240 sp. were synchronous
#id<-sum(df1$nint == df1$nind) # 86 sp. were indep.
