...
s1_row <- tab32[tab32$term == "abs.tot.td.pr5.sig", ]

# Extract the values
slope_med <- s1_row$slope_med
slope_lo  <- s1_row$slope_lo
slope_hi  <- s1_row$slope_hi
p_boot    <- s1_row$p_boot

# Create annotation label
label_text <- sprintf("Phylo slope 95%% CI: [%.2f, %.2f]\nMedian = %.2f, p_boot = %.3f",
                      slope_lo, slope_hi, slope_med, p_boot)


# Filter the row for the slope term of interest
s1_row <- df[df$term == "abs.tot.td.pr5.sig", ]
phylo_sum <- xall %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(
    draws = dplyr::n(),
    slope_med = median(abs.tot.td.pr5.sig, na.rm = TRUE),
    slope_lo  = quantile(abs.tot.td.pr5.sig, 0.025, na.rm = TRUE),
    slope_hi  = quantile(abs.tot.td.pr5.sig, 0.975, na.rm = TRUE),
    p_boot    = 2 * pmin(mean(abs.tot.td.pr5.sig >= 0), mean(abs.tot.td.pr5.sig <= 0)), # two-sided
    .groups = "drop"
  )
...
g1_finalP <- g1P +                          # your current layered plot object
