...
dfsig95.40.abs <- dfsig95.40 %>%
  dplyr::select(abs.avg.td.tas5.sig, ftas5.sig,
...
#==================== read bootstrap draws: Temperature model ====================
...
# Adjusted prediction:
# Temperature varies, HWI fixed at year-specific mean
pred_df_T <- make_adjusted_pred(
  data = dfsig.abs,
  boot_draws = xall,
  xvar = "abs.avg.td.tas5.sig",
  covar = "HWI"
)
...
label_T <- make_label_df(
  file40 = "RESULTS/model_phylolm_sig95_0-250km/model_tas5_abs_td/boot_summary.txt",
  file36 = "RESULTS/model_phylolm_sig95_0-250km_min36yr/model_tas5_abs_td/boot_summary.txt",
  file32 = "RESULTS/model_phylolm_sig95_0-250km_min32yr/model_tas5_abs_td/boot_summary.txt",
...
# Adjusted prediction:
# HWI varies, Temperature fixed at year-specific mean
pred_df_HWI <- make_adjusted_pred(
  data = dfsig.abs,
  boot_draws = xall,
  xvar = "HWI",
  covar = "abs.avg.td.tas5.sig"
)
...
pdf(here("Results/plot_regression_0-250km_40to32years_absTaildep_T.pdf"), width = 18, height = 12)
grid.arrange(g1T, g1T.hwi)
dev.off()

# caption: 
#   Figure X. Relationships between average absolute tail-dependent spatial synchrony in abundance and 
# average absolute tail-dependent spatial synchrony in Temperature (top row) and Hand–Wing Index (HWI; bottom row) 
...
# at its year-specific sample mean (i.e., Temperature effects were evaluated at mean HWI and HWI effects at mean Temperature synchrony). 
...
# and bootstrap p-value from the multivariable phylogenetic regression model:  Abundance synchrony ~ Temperature synchrony + HWI.
