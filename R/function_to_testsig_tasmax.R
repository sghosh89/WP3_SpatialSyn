...
#===================================
dfab<-readRDS(here(paste("RESULTS/abundance_spatsyn_nbin_",nbin,"_corlmcoru_sigres_summary.RDS",sep="")))
dfab<-dfab%>%filter(Lsig75!=0 | Usig75!=0)# 59 species
dfab<-dfab%>%dplyr::select(AOU,tail,Lsig75ab=Lsig75,Usig75ab=Usig75)

dftasmax<-readRDS(here(paste("RESULTS/tasmax_spatsyn_nbin_",nbin,"_corlmcoru_sigres_summary.RDS",sep="")))
dftasmax<-dftasmax%>%dplyr::select(AOU,Lsig75tasmax=Lsig75,Usig75tasmax=Usig75)

dfab<-left_join(dfab,dftasmax,by="AOU")
dfab$fab.sig<-(dfab$Lsig75ab+dfab$Usig75ab)/(dfab$Lsig75ab+abs(dfab$Usig75ab))
dfab$ftasmax.sig<-(dfab$Lsig75tasmax+dfab$Usig75tasmax)/(dfab$Lsig75tasmax+abs(dfab$Usig75tasmax))


tail75<-(dfab$Lsig75ab+dfab$Usig75ab)
dfab$tail75<-ifelse(tail75<0,"UT","LT")
dfab$tail75<-as.factor(dfab$tail75)

#ggplot(dfab,aes(col=tail75,y=(Lsig75ab+Usig75ab),x=(Lsig75tasmax+Usig75tasmax)))+
#  geom_point()+theme_bw()+geom_smooth(method="loess")+
#  stat_cor(aes(label = paste(..r.label.., ..p.label.., sep = "*`,`~")),
#           #label.x = -0.7, label.y = 0.8,
#           col="black")

ggplot(dfab,aes(col=tail75,y=fab.sig,
                x=ftasmax.sig))+
  geom_point()+theme_bw()+geom_smooth(method="loess")+
...
write.csv(dfab,here(paste("RESULTS/abundance_spatsyn_nbin_",nbin,"_tail75sig_summary.csv",sep="")), row.names = F)

#=====================================
dfab<-readRDS(here(paste("RESULTS/abundance_spatsyn_nbin_",nbin,"_corlmcoru_sigres_summary.RDS",sep="")))
dfab<-dfab%>%filter(Lsig95!=0 | Usig95!=0)# 35 species
dfab<-dfab%>%dplyr::select(AOU,tail,Lsig95ab=Lsig95,Usig95ab=Usig95)
dftasmax<-readRDS(here(paste("RESULTS/tasmax_spatsyn_nbin_",nbin,"_corlmcoru_sigres_summary.RDS",sep="")))
dftasmax<-dftasmax%>%dplyr::select(AOU,Lsig95tasmax=Lsig95,Usig95tasmax=Usig95)

dfab<-left_join(dfab,dftasmax,by="AOU")
dfab$fab.sig<-(dfab$Lsig95ab+dfab$Usig95ab)/(dfab$Lsig95ab+abs(dfab$Usig95ab))
dfab$ftasmax.sig<-(dfab$Lsig95tasmax+dfab$Usig95tasmax)/(dfab$Lsig95tasmax+abs(dfab$Usig95tasmax))


tail95<-(dfab$Lsig95ab+dfab$Usig95ab)
dfab$tail95<-ifelse(tail95<0,"UT","LT")
dfab$tail95<-as.factor(dfab$tail95)

# 20 data points to plot: 9 LT95, 11 UT95
ggplot(dfab,aes(col=tail95,y=fab.sig,
                x=ftasmax.sig))+
  geom_point()+theme_bw()+geom_smooth(method="loess")+
  geom_smooth(method="lm")+
  stat_cor(aes(label = paste(..r.label.., ..p.label.., sep = "*`,`~")),
...
write.csv(dfab,here(paste("RESULTS/abundance_spatsyn_nbin_",nbin,"_tail95sig_summary.csv",sep="")), row.names = F)
