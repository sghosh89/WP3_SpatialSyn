...
prepare_abund_data<-function(givenAOU){
  ...
  resloc<-paste("../RESULTS/AOU_", givenAOU,sep="")
  if(!dir.exists(resloc)){
    dir.create(resloc)
  }
  
  xroutes<-read.csv("../DATA/for_BBS/wrangled_data/uRID_lonlat_stratum.csv")
  abund_array<-readRDS("../DATA/for_BBS/wrangled_data/data1997to2019_abundance_array.RDS")
  #dim(abund_array) # year by site by species
  
  distm<-readRDS("../DATA/for_BBS/wrangled_data/pairwise_distance_km.RDS")
...
givenAOU<-"5460"
prepare_abund_data(givenAOU = givenAOU)
