...
#==============================================================================================================
boot32 <- readRDS(here("RESULTS/model_phylolm_sig95_0-250km_min32yr/model_tas5/model_est_phylolm_boot.RDS"))
boot36 <- readRDS(here("RESULTS/model_phylolm_sig95_0-250km_min36yr/model_tas5/model_est_phylolm_boot.RDS"))
boot40 <- readRDS(here("RESULTS/model_phylolm_sig95_0-250km/model_tas5/model_est_phylolm_boot.RDS"))
...
g2.wEq<-ggplot(data=dfsig.abs,
  ...
  stat_cor(aes(col=year,
               label = paste(..r.label..,..rr.label.., ..p.label.., 
                             sep = "*`,`~")),
           label.x = c(0,0,0), label.y = c(0.9,0.9,0.9), show.legend = F)+
  stat_regline_equation(label.x = c(0,0,0), label.y = c(1,1,1),
...
x_ranges <- dfsig.abs %>%
  group_by(year) %>%
  summarise(
    x_min = min(ftas5.sig, na.rm = TRUE),
    x_max = max(ftas5.sig, na.rm = TRUE),
    .groups = "drop"
  )
...
tab32<-read.table(here("RESULTS/model_phylolm_sig95_0-250km_min32yr/model_tas5/boot_summary.txt"), header=T, nrows=3)
tab36<-read.table(here("RESULTS/model_phylolm_sig95_0-250km_min36yr/model_tas5/boot_summary.txt"), header=T, nrows=3)
tab40<-read.table(here("RESULTS/model_phylolm_sig95_0-250km/model_tas5/boot_summary.txt"), header=T, nrows=3)

s40 <- tab40[tab40$term == "ftas5.sig", ]
s36 <- tab36[tab36$term == "ftas5.sig", ]
s32 <- tab32[tab32$term == "ftas5.sig", ]
...
pdf(here("Results/plot_regression_0-250km_40to32years_netTaildep_T_ref.pdf"),width=18, height=6)
print(g2T.wEq)
dev.off()

pdf(here("Results/plot_regression_0-250km_40to32years_netTaildepT.pdf"),width=18, height=6)
print(g2T)
dev.off()
