#------------------------------------------------------
# As per suggestions to lower the yr_threshold to include more routes
#rm(list=ls())
#-----------------------
library(here)
library(dplyr)
...
# 1018 routes are sampled atleast for 32 years, we will consider only those
# 575 routes are sampled atleast for 36 years, we will consider only those
#==============================================================================================================================
####################################   YOU CAN DIRECTLY START FROM HERE #######################################
#===============================================================================================================================
fshort<-readRDS(here("DATA/for_BBS/wrangled_data/data1979to2019_consistentprotocol.RDS"))

yr_threshold<-32 # we will consider only those routes (i.e., sites) sampled min 32 years
# we will vary from 32 to 36 to 40
...
route_Nyrs_metadata<-left_join(c2,xroutes,by="uRID")
saveRDS(route_Nyrs_metadata,here(paste("DATA/for_BBS/wrangled_data/route_",yr_threshold,"yrs_metadata.RDS", sep="")))

# Now we need to compute the pairwise geographic distance of these sites
library(geosphere)
dist<-distm(x=route_Nyrs_metadata[,c("Longitude","Latitude")], fun=distGeo)
dist<-dist/1000 # distance in Km
dim(dist)
rownames(dist)<-colnames(dist)<-route_Nyrs_metadata$uRID
# dist is a symmetric matrix, with pairwise distance between sites, in Km
range(dist)
dist[upper.tri(dist, diag=T)]<-NA
saveRDS(dist,here(paste("DATA/for_BBS/wrangled_data/pairwise_distance_km_min",yr_threshold,"yrs.RDS",sep="")))
...
fshort<-readRDS(here("DATA/for_BBS/wrangled_data/data1979to2019_consistentprotocol.RDS"))
fshort$uRID<-paste(fshort$CountryNum,"_",fshort$StateNum,"_",fshort$Route,sep="")
fshort_Nyr<-fshort%>%filter(uRID%in%route_Nyrs_metadata$uRID)
saveRDS(fshort_Nyr,here(paste("DATA/for_BBS/wrangled_data/data1979to2019_consistentprotocol_min",yr_threshold,"yr.RDS",sep="")))

nsp<-length(unique(fshort_Nyr$AOU))# 
nyr<-length(unique(fshort_Nyr$Year)) # 
range(fshort_Nyr$Year)# 1979-2019
length(unique(fshort$uRID))#5089
nsite<-length(unique(fshort_Nyr$uRID)) 
usp<-sort(unique(fshort_Nyr$AOU)) 

abund_array<-array(data=0, dim=c(nyr,nsite,nsp))
dimnames(abund_array)[[1]]<- as.character(sort(unique(fshort_Nyr$Year)))
dimnames(abund_array)[[2]]<- route_Nyrs_metadata$uRID
...
for(i in 1:nsp){
  ...
  sp<-usp[i]
  zz<-fshort_Nyr%>%filter(AOU==sp)%>%dplyr::select(uRID,Year,Stop1to50)
...
dim(abund_array)# year by site by species = 41 by 1018 by 615 for yr_threshold = 32;
#                                         = 41 by 575 by 564 for yr_threshold = 36;
#                                         = 41 by 161 by 498 for yr_threshold = 40;
...
if(yr_threshold==40){
  ...
  saveRDS(abund_array,here(paste("DATA/for_BBS/wrangled_data/data1979to2019_abundance_array_min",yr_threshold,"yr.RDS",sep="")))
  write.csv(species_absentinfo,here(paste("DATA/for_BBS/wrangled_data/data1979to2019_abundance_species_absentinfo_min",yr_threshold,"yr.csv",sep="")), row.names = F)
}
