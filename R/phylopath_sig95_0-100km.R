...
if(!dir.exists(here("RESULTS/model_phylopath_sig95_0-100km"))){
  dir.create(here("RESULTS/model_phylopath_sig95_0-100km"))
}
...
saveRDS(ct3,here("RESULTS/model_phylopath_sig95_0-100km/consensus_tree_with_edgelength.RDS"))
...
pdf(here("RESULTS/model_phylopath_sig95_0-100km/species_phylogeny_0_100km_absolute_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g1 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_0-100km/species_phylogeny_0_100km_fLU_ab.pdf"), width = 9, height = 9) # Open a new pdf file
g2 # Write the grid.arrange in the file
dev.off()

pdf(here("RESULTS/model_phylopath_sig95_0-100km/species_phylogeny_0_100km_HWI.pdf"), width = 9, height = 9) # Open a new pdf file
g3 # Write the grid.arrange in the file
dev.off()

#============== model ===========
call_phylopath_sig95_0_100km<-function(model, df){
  ...
  if(model=="tas"){
    ...
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_tas")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5"){
    modelsHWI_Tonly<-model_tas5
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_tas5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr"){
    modelsHWI_Tonly<-model_pr
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_pr")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5"){
    modelsHWI_Tonly<-model_pr5
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_pr5")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="tas5_abs_td"){
    modelsHWI_Tonly<-model_tas5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_tas5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else if(model=="pr5_abs_td"){
    modelsHWI_Tonly<-model_pr5_abs_td
    myresloc<-here("RESULTS/model_phylopath_sig95_0-100km/model_pr5_abs_td")
    if(!dir.exists(myresloc)){dir.create(myresloc)}
  }else{
    print("define model")
  }
