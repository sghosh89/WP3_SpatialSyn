...
testsig_tas_Nyrs_threshold<-function(numsurr=1000, nbin=4, chosen_rad=c(0,250), yr_threshold=40){
  ...
  df_tas<-df_tas%>%dplyr::select(AOU,nL,nU,L,U)
  
  df<-df%>%dplyr::select(AOU,fLU_tas)
  df<-left_join(df,df_tas,by="AOU") # this is summarized value within 0-250Km distance
  ...
  for(spi in 1:nrow(df)){
    ...
    nps<-readRDS(here(paste(inputdistloc,"/tas_spatsyn_nbin_",nbin,
    ...
    dat_detrend<-readRDS(here(paste(inputdistloc,"/year_by_site_tas_detrended.RDS",sep="")))
    ...
    saveRDS(dfid,here(paste(inputdistloc,"/tas_spatsyn_nbin_",nbin,"/corlmcoru_sigres.RDS",sep="")))
...
testsig_tas_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 32)
testsig_tas_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 36)
testsig_tas_Nyrs_threshold(numsurr = 1000, nbin=4, chosen_rad = c(0,250), yr_threshold = 40)
